...
// Processor.
func NewBlockChain(chainDb ethdb.Database, config *params.ChainConfig, engine consensus.Engine, eventPool *event.FeedPool, vmConfig vm.Config) (*BlockChain, error) {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if len(diff) > 0 {
		go bc.eventPool.Send(RemovedTransactionEvent{diff})
	}
	if len(deletedLogs) > 0 {
		go bc.eventPool.Send(RemovedLogsEvent{deletedLogs})
	}

	if len(oldChain) > 0 {
		go func() {
			for _, block := range oldChain {
				bc.eventPool.Send(ChainSideEvent{Block: block})
			}
		}()
	}
...
// posts them into the event pool.
func (bc *BlockChain) postChainEvents(events []interface{}, logs []*types.Log) {
	// post event logs for further processing
	bc.eventPool.Send(logs)
	for _, event := range events {
		if event, ok := event.(ChainEvent); ok {
			...
			if bc.LastBlockHash() == event.Hash {
				bc.eventPool.Send(ChainHeadEvent{event.Block})
			}
		}
		// Fire the insertion events individually too
		bc.eventPool.Send(event)
	}
}
