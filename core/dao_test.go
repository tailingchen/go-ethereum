...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, proConf, ethash.NewFaker(), new(event.FeedPool), vm.Config{})

	conDb, _ := ethdb.NewMemDatabase()
	gspec.MustCommit(conDb)
	conConf := &params.ChainConfig{HomesteadBlock: big.NewInt(0), DAOForkBlock: forkBlock, DAOForkSupport: false}
	conBc, _ := NewBlockChain(conDb, conConf, ethash.NewFaker(), new(event.FeedPool), vm.Config{})
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db, _ = ethdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, conConf, ethash.NewFaker(), new(event.FeedPool), vm.Config{})
		...
		db, _ = ethdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, proConf, ethash.NewFaker(), new(event.FeedPool), vm.Config{})
