...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	var (
		...
		blockchain, _ = core.NewBlockChain(db, config, pow, evpool, vm.Config{})
	)
	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, 1000, evpool, new(testTxPool), pow, blockchain, db)
	if err != nil {
